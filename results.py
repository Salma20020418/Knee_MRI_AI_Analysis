...
# These should ideally be trained offline on validation data
# Format: [axial_weight, coronal_weight, sagittal_weight]
logreg_acl_weights = np.array([0.415, 0.257, 0.328])
logreg_meniscus_weights = np.array([0.375, 0.298, 0.327])
acl_intercept = -0.46
...
def classify_knee_condition_multiview(image_path_dict, save_path=None, row=None, col=None):
    ...
    if final_acl_prob < final_meniscus_prob - 0.09 and final_meniscus_prob > 0.48:
        meniscus_result = 0
    elif final_meniscus_prob < final_acl_prob - 0.1 and final_acl_prob > 0.5:
        acl_result = 0
...
axial_files = sorted(Path("F:\\Graduation_Project\\Sample\\try_test\\axial_normal").glob("*.png"))
coronal_files = sorted(Path("F:\\Graduation_Project\\Sample\\try_test\\coronal_normal").glob("*.png"))
sagittal_files = sorted(Path("F:\\Graduation_Project\\Sample\\try_test\\sagittal_normal").glob("*.png"))
